      ...
      <div class="col-md-3">
        <div class="form-group">
          ...
          <select id="selectSetor" class="form-control" v-model="departamentoFiltrado">
            ...
            <option
              v-for="departamento of departamentosFiltrados"
              :key="departamento.sigla"
      ...
      <div class="col-md-6">
        <div class="form-group">
          ...
          <select id="selectSupervisor" class="form-control" v-model="supervisorFiltrado">
            <option value>Todos</option>
            <option
              v-for="supervisor of supervisoresSort"
              :key="supervisor.rf"
        ...
        <table class="table table-bordered table-hover">
          ...
          <tbody>
            <tr v-for="(vaga, indice) of vagasPorDepartamento" :key="vaga.id">
              ...
              <td>
                <b-link
                  href="#" id="modalHistoricoVagas" style="cursor: pointer;"
                  @click="abrirModalHistoricoVagas(vaga.id)"
                >{{ vaga.id }}</b-link>
              </td>
              <td>{{ vaga.status }}</td>
          ...
          <b-modal ref="modalHistoricoVagas" hide-footer title="Histórico da vaga" size="lg">
            <div class="d-block text-center">
              ...
              <table class="table table-bordered table-hover">
                ...
                <tbody>
                  <tr
                    v-for="(historicoEstagiarios, indice) of historicoVagas"
            ...
            <b-button
              ...
              block
              @click="fecharModalHistoricoVagas"
            >Fechar</b-button>
          </b-modal>
        </div>
      </template>
      ...
      <table class="table table-bordered table-hover">
        ...
        <tbody>
          <tr v-for="(vaga, indice) of vagasPorDepartamento" :key="vaga.id">
            <td>{{ indice + 1 }}</td>
            <td>
              <a v-b-modal.modalHistoricoVagas>{{ vaga.id }}</a>
            </td>
            <td>{{ vaga.status }}</td>
            <td>{{ vaga.dep_hierarquico }}</td>
            <td>{{ vaga.supervisor }}</td>
  ...
  computed: {
    ...
    vagasPorDepartamento() {
      ...
      if (
        ...
        this.departamentoFiltrado === "" &&
        this.supervisorFiltrado === "" &&
        this.status === "Ocupadas"
      ) {
        return this.vagasOcupadas;
      } else if (
        this.departamentoFiltrado === "" &&
        this.supervisorFiltrado === "" &&
        this.status === "Em seleção"
      ) {
        return this.vagasEmSelecao;
      } else if (
        this.departamentoFiltrado === "" &&
        this.supervisorFiltrado === "" &&
        this.status === "TRANSFERIDA.SMG"
      ) {
        return this.vagasTransferidas;
      } else if (
        this.departamentoFiltrado === "" &&
        this.supervisorFiltrado === "" &&
        this.status === "Canceladas"
      ) {
      ...
      // filtro somente por supervisor e status
        ...
        let vagas = this.vagas.filter(vaga => {
          return (
            vaga.supervisor === this.supervisorFiltrado &&
            vaga.status === "OCUPADA"
          );
        });
        return vagas;
      ...
      // filtro somente por supervisor e departamento
        ...
        let vagas = this.vagas.filter(vaga => {
          return (
            vaga.supervisor === this.supervisorFiltrado &&
            vaga.dep_hierarquico === this.departamentoFiltrado
          );
        });
        return vagas;
      }
      ...
      // filtro por departamento, supervisor e status
        ...
        this.departamentoFiltrado != "" &&
        (this.supervisorFiltrado != "") & (this.status === "Livres")
      ) {
        let vagas = this.vagas.filter(vaga => {
          return (
            vaga.dep_hierarquico === this.departamentoFiltrado &&
        ...
        this.departamentoFiltrado != "" &&
        (this.supervisorFiltrado != "") & (this.status === "Ocupadas")
      ) {
        let vagas = this.vagas.filter(vaga => {
          return (
            vaga.dep_hierarquico === this.departamentoFiltrado &&
        ...
        this.departamentoFiltrado != "" &&
        (this.supervisorFiltrado != "") & (this.status === "Em seleção")
      ) {
        let vagas = this.vagas.filter(vaga => {
          return (
            vaga.dep_hierarquico === this.departamentoFiltrado &&
        ...
        this.departamentoFiltrado != "" &&
        (this.supervisorFiltrado != "") & (this.status === "Canceladas")
      ) {
        let vagas = this.vagas.filter(vaga => {
          return (
            vaga.dep_hierarquico === this.departamentoFiltrado &&
        ...
        this.departamentoFiltrado != "" &&
        (this.supervisorFiltrado != "") & (this.status === "TRANSFERIDA.SMG")
      ) {
        let vagas = this.vagas.filter(vaga => {
          return (
            vaga.dep_hierarquico === this.departamentoFiltrado &&
    ...
    acrescentaSupervisorEEstagiarioVaga() {
      for (let i in this.estagiarios) {
        for (let k in this.vagas) {
          if (
            this.estagiarios[i].cod_vaga === this.vagas[k].id &&
            this.estagiarios[i].situacao == 1
          ) {
            this.vagas[k].supervisor = this.estagiarios[
              i
