<template>
  <div>
    ...
    <b-modal ref="consulta-estagiario" hide-footer title="Consulta de Estagiários">
      ...
      <b-form ref="form" @submit.prevent="validaCpf" @keyup.enter="buscaEstagiario">
        ...
        <the-mask
          ...
          v-model="post.cpf"
        ></the-mask>
        <div v-if="cpfInvalido" class="invalid-feedback">O CPF deve ter 11 números</div>
    ...
    <!--Modal acionado pelo componente Botoes.vue-->

    <template v-if="mostrarConteudoConsulta">
      <div v-if="msg.success" class="alert alert-success mt-2">{{ msg.sucesso }}</div>

      <div v-if="msg.error" class="alert alert-danger mt-2">{{ msg.erro }}</div>

      <div v-if="msg.vagaAlterada" class="alert alert-success">{{ msg.vagaSuccess }}</div>

      <div v-if="msg.vagaNaoAlterada" class="alert alert-danger">{{ msg.vagaError }}</div>
      ...
      <b-card no-body>
        <b-tabs card>
          ...
          <b-tab title="Recesso">
            <b-card-text>
              <recesso-atualizar :post="post" :inserirEstagiario="inserirEstagiario" />
            </b-card-text>
          </b-tab>
        </b-tabs>
      </b-card>
...
export default {
  ...
  computed: {
    EstagiariosComFiltro() {
      ...
      if (this.filtro) {
        ...
        this.estagiarioFiltrado = this.estagiarioFiltrado.filter(estagiario =>
          exp.test(estagiario.nome)
        );
  ...
  methods: {
    ...
    insereDataDesligamento() {
      ...
      const dia =
        dataHoje.getDate() < 10
          ...
          : `${dataHoje.getDate()}`;
      const mes =
        dataHoje.getMonth() + 1 < 10
          ? `0${dataHoje.getMonth() + 1}`
          : `${dataHoje.getMonth() + 1}`;
    ...
    validaNacionalidade(nacionalidade) {
    ...
    converteTermosAditivos() {
      if (this.post.dt_inicio_1_aditivo) {
        this.post.dt_inicio_1_aditivo = this.post.dt_inicio_1_aditivo.substr(
          0,
          10
        );
      }
      if (this.post.dt_inicio_2_aditivo) {
        this.post.dt_inicio_2_aditivo = this.post.dt_inicio_2_aditivo.substr(
          0,
          10
        );
      }
      if (this.post.dt_inicio_3_aditivo) {
        this.post.dt_inicio_3_aditivo = this.post.dt_inicio_3_aditivo.substr(
          0,
          10
        );
      }
      if (this.post.dt_termino_1_aditivo) {
        this.post.dt_termino_1_aditivo = this.post.dt_termino_1_aditivo.substr(
          0,
          10
        );
      }
      if (this.post.dt_termino_2_aditivo) {
        this.post.dt_termino_2_aditivo = this.post.dt_termino_2_aditivo.substr(
          0,
          10
        );
      }
      if (this.post.dt_termino_3_aditivo) {
        this.post.dt_termino_3_aditivo = this.post.dt_termino_3_aditivo.substr(
          0,
          10
        );
      }
    ...
    converteDataInicialLauda() {
      if (this.post.dt_termino_inicial_lauda) {
        this.post.dt_termino_inicial_lauda = this.post.dt_termino_inicial_lauda.substr(
          0,
          10
        );
      }
    ...
    converteSemestreDesligamento() {
      if (this.post.semestre_desligamento) {
        this.post.semestre_desligamento = this.post.semestre_desligamento.substr(
          0,
          10
        );
      }
    ...
    converteRecessos() {
      if (this.post.dt_inicial_1) {
        this.post.dt_inicial_1 = this.post.dt_inicial_1.substr(0, 10);
      }
      if (this.post.dt_inicial_2) {
        this.post.dt_inicial_2 = this.post.dt_inicial_2.substr(0, 10);
      }
      if (this.post.dt_inicial_3) {
        this.post.dt_inicial_3 = this.post.dt_inicial_3.substr(0, 10);
      }
      if (this.post.dt_inicial_4) {
        this.post.dt_inicial_4 = this.post.dt_inicial_4.substr(0, 10);
      }
      ...
      if (this.post.dt_termino_1) {
        this.post.dt_termino_1 = this.post.dt_termino_1.substr(0, 10);
      }
      if (this.post.dt_termino_2) {
        this.post.dt_termino_2 = this.post.dt_termino_2.substr(0, 10);
      }
      if (this.post.dt_termino_3) {
        this.post.dt_termino_3 = this.post.dt_termino_3.substr(0, 10);
      }
      if (this.post.dt_termino_4) {
        this.post.dt_termino_4 = this.post.dt_termino_4.substr(0, 10);
      }
      if (this.post.dt_termino_5) {
        this.post.dt_termino_5 = this.post.dt_termino_5.substr(0, 10);
      }
      if (this.post.dt_termino_6) {
        this.post.dt_termino_6 = this.post.dt_termino_6.substr(0, 10);
      }
      if (this.post.dt_termino_7) {
        this.post.dt_termino_7 = this.post.dt_termino_7.substr(0, 10);
      }
    },
    buscaEstagiario() {
      this.loading = true;
    ...
    alteraStatusVaga() {
      ...
      this.axios
        .patch(uriVagas, this.statusVaga)
        .then(response => {
    ...
    converteDatas() {
      if (this.post.horario_entrada) {
        this.post.horario_entrada =
          this.post.horario_entrada.length == 8
            ? `1899-12-30 ${this.post.horario_entrada}`
            : this.post.horario_entrada;
      }
      if (this.post.horario_saida) {
        this.post.horario_saida =
          this.post.horario_saida.length == 8
            ? `1899-12-30 ${this.post.horario_saida}`
            : this.post.horario_saida;
      }
      if (this.post.dt_inicio) {
        this.post.dt_inicio == `${this.post.dt_inicio.substr(0, 10)} 00:00:00`
          ? this.post.dt_inicio
          : (this.post.dt_inicio += " 00:00:00");
      }
      if (this.post.dt_termino) {
        this.post.dt_termino == `${this.post.dt_termino.substr(0, 10)} 00:00:00`
          ? this.post.dt_termino
          : (this.post.dt_termino += " 00:00:00");
      }
      if (this.post.dt_termino) {
        this.post.dt_termino_inicial_lauda = this.post.dt_termino;
      }
      if (this.post.dt_inicial_1) {
        this.post.dt_inicial_1 ==
        `${this.post.dt_inicial_1.substr(0, 10)} 00:00:00`
          ? this.post.dt_inicial_1
          : (this.post.dt_inicial_1 += " 00:00:00");
      }

      if (this.post.dt_inicial_2) {
        this.post.dt_inicial_2 ==
        `${this.post.dt_inicial_2.substr(0, 10)} 00:00:00`
          ? this.post.dt_inicial_2
          : (this.post.dt_inicial_2 += " 00:00:00");
      }

      if (this.post.dt_inicial_3) {
        this.post.dt_inicial_3 ==
        `${this.post.dt_inicial_3.substr(0, 10)} 00:00:00`
          ? this.post.dt_inicial_3
          : (this.post.dt_inicial_3 += " 00:00:00");
      }

      if (this.post.dt_inicial_4) {
        this.post.dt_inicial_4 ==
        `${this.post.dt_inicial_4.substr(0, 10)} 00:00:00`
          ? this.post.dt_inicial_4
          : (this.post.dt_inicial_4 += " 00:00:00");
      }

      if (this.post.dt_inicial_5) {
        this.post.dt_inicial_5 ==
        `${this.post.dt_inicial_5.substr(0, 10)} 00:00:00`
          ? this.post.dt_inicial_5
          : (this.post.dt_inicial_5 += " 00:00:00");
      }

      if (this.post.dt_inicial_6) {
        this.post.dt_inicial_6 ==
        `${this.post.dt_inicial_6.substr(0, 10)} 00:00:00`
          ? this.post.dt_inicial_6
          : (this.post.dt_inicial_6 += " 00:00:00");
      }

      if (this.post.dt_inicial_7) {
        this.post.dt_inicial_7 ==
        `${this.post.dt_inicial_7.substr(0, 10)} 00:00:00`
          ? this.post.dt_inicial_7
          : (this.post.dt_inicial_7 += " 00:00:00");
      }

      if (this.post.dt_inicio_1_aditivo) {
        this.post.dt_inicio_1_aditivo ==
        `${this.post.dt_inicio_1_aditivo.substr(0, 10)} 00:00:00`
          ? this.post.dt_inicio_1_aditivo
          : (this.post.dt_inicio_1_aditivo += " 00:00:00");
      }

      if (this.post.dt_inicio_2_aditivo) {
        this.post.dt_inicio_2_aditivo ==
        `${this.post.dt_inicio_2_aditivo.substr(0, 10)} 00:00:00`
          ? this.post.dt_inicio_2_aditivo
          : (this.post.dt_inicio_2_aditivo += " 00:00:00");
      }

      if (this.post.dt_termino_1_aditivo) {
        this.post.dt_termino_1_aditivo ==
        `${this.post.dt_termino_1_aditivo.substr(0, 10)} 00:00:00`
          ? this.post.dt_termino_1_aditivo
          : (this.post.dt_termino_1_aditivo += " 00:00:00");
      }

      if (this.post.dt_termino_2_aditivo) {
        this.post.dt_termino_2_aditivo ==
        `${this.post.dt_termino_2_aditivo.substr(0, 10)} 00:00:00`
          ? this.post.dt_termino_2_aditivo
          : (this.post.dt_termino_2_aditivo += " 00:00:00");
      }

      if (this.post.dt_termino_1) {
        this.post.dt_termino_1 ==
        `${this.post.dt_termino_1.substr(0, 10)} 00:00:00`
          ? this.post.dt_termino_1
          : (this.post.dt_termino_1 += " 00:00:00");
      }

      if (this.post.dt_termino_2) {
        this.post.dt_termino_2 ==
        `${this.post.dt_termino_2.substr(0, 10)} 00:00:00`
          ? this.post.dt_termino_2
          : (this.post.dt_termino_2 += " 00:00:00");
      }

      if (this.post.dt_termino_3) {
        this.post.dt_termino_3 ==
        `${this.post.dt_termino_3.substr(0, 10)} 00:00:00`
          ? this.post.dt_termino_3
          : (this.post.dt_termino_3 += " 00:00:00");
      }

      if (this.post.dt_termino_4) {
        this.post.dt_termino_4 ==
        `${this.post.dt_termino_4.substr(0, 10)} 00:00:00`
          ? this.post.dt_termino_4
          : (this.post.dt_termino_4 += " 00:00:00");
      }

      if (this.post.dt_termino_5) {
        this.post.dt_termino_5 ==
        `${this.post.dt_termino_5.substr(0, 10)} 00:00:00`
          ? this.post.dt_termino_5
          : (this.post.dt_termino_5 += " 00:00:00");
      }

      if (this.post.dt_termino_6) {
        this.post.dt_termino_6 ==
        `${this.post.dt_termino_6.substr(0, 10)} 00:00:00`
          ? this.post.dt_termino_6
          : (this.post.dt_termino_6 += " 00:00:00");
      }

      if (this.post.dt_termino_7) {
        this.post.dt_termino_7 ==
        `${this.post.dt_termino_7.substr(0, 10)} 00:00:00`
          ? this.post.dt_termino_7
          : (this.post.dt_termino_7 += " 00:00:00");
      }

      if (this.post.data_nascimento) {
        this.post.data_nascimento ==
        `${this.post.data_nascimento.substr(0, 10)} 00:00:00`
          ? this.post.data_nascimento
          : (this.post.data_nascimento += " 00:00:00");
      }

      if (this.post.desligado) {
        this.post.desligado == `${this.post.desligado.substr(0, 10)} 00:00:00`
          ? this.post.desligado
          : (this.post.desligado += " 00:00:00");
      }
    },
    scrollTop() {
      window.scrollTo(0, 0);
    ...
    selectVaga() {
      if (this.post.cod_vaga) {
        ...
        this.axios
          .get(uriStatusVaga)
          .then(response => (this.statusVaga = response.data));
      }
    }
  }
};
